...
public class BleService extends Service {
    ...
    private final BluetoothGattExecutor mExecutor = new BluetoothGattExecutor() {
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                final AbstractSensor<?> sensor = TiSensors.getSensor(characteristic.getService()
                        .getUuid().toString());
                if (sensor != null) {
    ...
    private void broadcastUpdate(final String action,
        ...
        final AbstractSensor<?> sensor = TiSensors.getSensor(characteristic.getService().getUuid()
                .toString());
        if (sensor != null) {
    ...
    public boolean connect(final String address) {
        ...
        // autoConnect
        // parameter to false.
        mBluetoothGatt = device.connectGatt(this, false, mExecutor);
    ...
    public void updateSensor(AbstractSensor<?> sensor) {
        if (sensor == null)
            return;
        ...
        mExecutor.update(sensor);
        mExecutor.execute(mBluetoothGatt);
    }
     ...
     */
    public void enableSensor(AbstractSensor<?> sensor, boolean enabled) {
        if (sensor == null)
            return;
        ...
        mExecutor.enable(sensor, enabled);
        mExecutor.execute(mBluetoothGatt);
    }
